...
function createLightGroup(group, lights) {
    const { attributes: { light: { allOf = false } = {} } = {}, lights: includedLights } = group;
...
export function create(config, data) {
    const { 'light-groups': lightGroupsConfig } = config;
    const { lights } = data;
    Object.assign(data, {
...
function manageLightGroup(group, httpHookServer, lightGroupIntercepts, metricAggregates) {
    const { instance, name, attributes: { light: { timeout = 0, initialTimer = false } = {} } = {} } = group;
    if (Object.keys(lightGroupIntercepts).includes(name)) {
        const { [name]: intercept } = lightGroupIntercepts;
        const globalMeanAggregateSensor = metricAggregates.find(({ group: aggregateGroup, metric, type }) => {
        ...
        if (globalMeanAggregateSensor) {
            instance.setInterceptor((on, instances) => {
                ...
                return instances.map((light, index) => {
                    ...
                    if (brightness >= from && to === null)
                        return light.setPower(true);
                    if (from === null && brightness <= to)
                        return light.setPower(true);
...
function groupWithDoorSensor(lightGroups, doorSensors) {
    coupleDoorSensorToLight(lightGroups, doorSensors, 'duschbadLamps', 'duschbadDoor');
    coupleDoorSensorToLight(lightGroups, doorSensors, 'wannenbadLamps', 'wannenbadDoor');
}
function groupWithRfSwitch(lightGroups, rfSwitches) {
    //  DUSCHBAD
    //    wall switches
    coupleRfSwitchToLight(lightGroups, rfSwitches, 'duschbadLamps', 'duschbadWallDoor', 1);
    //  ESSZIMMER
    //    buttons
    coupleRfSwitchToLight(lightGroups, rfSwitches, 'wohnzimmerRelated', 'esszimmerButton1', 4);
    //  FLUR
    //    wall switches
    coupleRfSwitchToLight(lightGroups, rfSwitches, 'flurDeckenlampe', 'flurWallFront', 1);
    coupleRfSwitchToLight(lightGroups, rfSwitches, 'flurDeckenlampe', 'flurWallMiddle', 1);
    coupleRfSwitchToLight(lightGroups, rfSwitches, 'flurDeckenlampe', 'flurWallBack', 1);
    //  KUECHE
    //    wall switches
    coupleRfSwitchToLight(lightGroups, rfSwitches, 'kuecheLed', 'kuecheWallLeft', 1);
    coupleRfSwitchToLight(lightGroups, rfSwitches, 'kuecheLed', 'kuecheWallRight', 1);
    //    buttons
    coupleRfSwitchToLight(lightGroups, rfSwitches, 'kuecheLed', 'kuecheButtonLeft', 4);
    //  WANNENBAD
    //    wall switches
    coupleRfSwitchToLight(lightGroups, rfSwitches, 'wannenbadLamps', 'wannenbadWallDoor', 1);
    //  WOHNZIMMER
    //    buttons
    coupleRfSwitchToLight(lightGroups, rfSwitches, 'wohnzimmerRelated', 'wohnzimmerButton1', 4);
}
...
function lightGroupHmi(group, hmiServer) {
    const { name, instance, timer, attributes: { hmi: hmiDefaults = null } = {} } = group;
    if (!hmiDefaults)
        return;
...
export function manage(config, data) {
    const { globals: { lightGroupIntercepts } } = config;
    const { allLightsGroup, doorSensors, hmiServer, httpHookServer, lightGroups, metricAggregates, rfSwitches } = data;
...
//# sourceMappingURL=light-groups.js.map