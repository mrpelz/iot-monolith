...
import { coupleDoorSensorToLight, coupleDoorSensorToLightTimeout, coupleRfSwitchToLight, coupleRfSwitchToLightIncrease, coupleRfSwitchesToLightPermutations } from '../utils/lights.js';
...
function addPersistenceHandler(name, instance, lightDb, dbKey, instanceKey) {
    ...
    const { [dbKey]: value = false } = lightDb[name] || {};
...
function createLedLightInstance(options, driver) {
    const { duration, gamma, steps, useChannel } = options;
    try {
        return new LedLight({
            driver,
            duration,
...
export function create(config, data) {
    ...
    const lightDrivers = driversConfig.map((options) => {
        const { disable: driverDisable = false, host, lights: lightsConfig = [], name: driverName, type } = options;
        if (driverDisable || !driverName || !type)
            return null;
        const lightsConfigFiltered = lightsConfig.filter(({ disable = false, name }) => {
            return name && !disable;
        });
        if (!lightsConfigFiltered.length)
            return null;
        let driver;
        switch (type) {
            case 'SONOFF_BASIC':
...
function manageRelayLight(options, httpHookServer) {
    const { instance: driver, lights = [], attributes: { driver: { enableButton = false } = {} } = {} } = options;
    driver.on('reliableConnect', () => {
        resolveAlways(driver.indicatorBlink(5, true));
    });
    lights.forEach((light, index) => {
        const { instance, name, attributes: { light: { timeout = 0, initialTimer = false } = {} } = {} } = light;
        if (index === 0) {
            instance.on('change', () => {
        ...
        if (timeout) {
            ...
            instance.on('set', () => {
                if (!timer.isRunning || !instance.power)
                    return;
                timer.start();
            });
            instance.on('change', () => {
...
function lightWithDoorSensor(lights, doorSensors) {
    coupleDoorSensorToLight(lights, doorSensors, 'abstellraumDeckenlampe', 'abstellraumDoor');
    coupleDoorSensorToLightTimeout(lights, doorSensors, 'flurDeckenlampeFront', 'entryDoor', 60000);
}
function lightWithRfSwitch(lights, rfSwitches) {
    //  ABSTELLRAUM
    //    wall switches
    coupleRfSwitchToLight(lights, rfSwitches, 'abstellraumDeckenlampe', 'abstellraumWall', 1);
    //  ARBEITSZIMMER
    //    wall switches
    coupleRfSwitchToLight(lights, rfSwitches, 'arbeitszimmerDeckenlampe', 'arbeitszimmerWall', 1);
    //  DUSCHBAD
    //    permutations
    coupleRfSwitchesToLightPermutations(lights, rfSwitches, [
        'duschbadDeckenlampe',
    ...
    //  ESSZIMMER
    //    wall switches
    coupleRfSwitchToLight(lights, rfSwitches, 'esszimmerDeckenlampe', 'esszimmerWallFront', 1);
    coupleRfSwitchToLight(lights, rfSwitches, 'wohnzimmerDeckenlampe', 'esszimmerWallFront', 2);
    //    buttons
    coupleRfSwitchToLight(lights, rfSwitches, 'esszimmerDeckenlampe', 'esszimmerMulti1', 1);
    coupleRfSwitchToLight(lights, rfSwitches, 'wohnzimmerKallaxLedWWhite', 'esszimmerMulti1', 2);
    coupleRfSwitchToLight(lights, rfSwitches, 'esszimmerFloodlight', 'esszimmerMulti1', 3);
    coupleRfSwitchToLight(lights, rfSwitches, 'esszimmerStehlampe', 'esszimmerMulti1', 4);
    //  KUECHE
    //    permutations
    coupleRfSwitchesToLightPermutations(lights, rfSwitches, [
        'kuecheLedLeftUp',
    ...
    //  SCHLAFZIMMER
    //    wall switches
    coupleRfSwitchToLight(lights, rfSwitches, 'schlafzimmerDeckenlampe', 'schlafzimmerWallRight', 1);
    //    permutations
    coupleRfSwitchesToLightPermutations(lights, rfSwitches, [
    ...
    //    buttons
    coupleRfSwitchToLightIncrease(lights, rfSwitches, 'schlafzimmerBedLedNightstandLeft', 'schlafzimmerButton1', 1);
    coupleRfSwitchToLightIncrease(lights, rfSwitches, 'schlafzimmerBedLedNightstandRight', 'schlafzimmerButton1', 2);
    coupleRfSwitchToLightIncrease(lights, rfSwitches, 'schlafzimmerBedLedWhite', 'schlafzimmerButton1', 3);
    coupleRfSwitchToLightIncrease(lights, rfSwitches, 'schlafzimmerBedLedFloor', 'schlafzimmerButton1', 4);
    coupleRfSwitchToLightIncrease(lights, rfSwitches, 'schlafzimmerBedLedNightstandLeft', 'schlafzimmerButton2', 1);
    coupleRfSwitchToLightIncrease(lights, rfSwitches, 'schlafzimmerBedLedNightstandRight', 'schlafzimmerButton2', 2);
    coupleRfSwitchToLightIncrease(lights, rfSwitches, 'schlafzimmerBedLedWhite', 'schlafzimmerButton2', 3);
    coupleRfSwitchToLightIncrease(lights, rfSwitches, 'schlafzimmerBedLedFloor', 'schlafzimmerButton2', 4);
    //  WANNENBAD
    //    permutations
    coupleRfSwitchesToLightPermutations(lights, rfSwitches, [
    ...
    //  WOHNZIMMER
    //    wall switches
    coupleRfSwitchToLight(lights, rfSwitches, 'wohnzimmerDeckenlampe', 'wohnzimmerWallBack', 1);
    coupleRfSwitchToLight(lights, rfSwitches, 'wohnzimmerStehlampe', 'wohnzimmerWallBack', 2);
    coupleRfSwitchToLight(lights, rfSwitches, 'esszimmerDeckenlampe', 'wohnzimmerWallBack', 3);
    // buttons
    coupleRfSwitchToLight(lights, rfSwitches, 'wohnzimmerDeckenlampe', 'wohnzimmerMulti1', 1);
    coupleRfSwitchToLight(lights, rfSwitches, 'esszimmerFloodlight', 'wohnzimmerMulti1', 2);
    coupleRfSwitchToLight(lights, rfSwitches, 'wohnzimmerKallaxLedWWhite', 'wohnzimmerMulti1', 3);
    coupleRfSwitchToLight(lights, rfSwitches, 'wohnzimmerStehlampe', 'wohnzimmerMulti1', 4);
}
...
function relayLightHmi(options, hmiServer) {
    ...
    lights.forEach((light) => {
        const { name, instance, timer, attributes: { hmi: hmiDefaults = null } = {} } = light;
        if (!hmiDefaults)
            return;
...
export function manage(_, data) {
    const { doorSensors, hmiServer, httpHookServer, lightDrivers, lights, prometheus, rfSwitches } = data;
