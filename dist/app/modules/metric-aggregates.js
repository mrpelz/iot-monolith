...
export function create(config, data) {
    const { 'metric-aggregates': metricAggregatesConfig } = config;
    ...
    const metricAggregates = flattenArrays(metricAggregatesConfig.map((group) => {
        ...
        const instances = roomSensors.filter((options) => {
            ...
            if (disable)
                return null;
            return (sensors.includes(sensorName)
                && metrics.every((m) => {
...
function metricAggregatesToPrometheus(metricAggregates, prometheus) {
    metricAggregates.forEach((aggregate) => {
        ...
        prometheus.metric(metric, {
            ...
            subType: type
        }, instance.get, instance.getTime);
    });
}
function metricAggrgatesHmi(metricAggregates, histories, hmiServer, unitMap, valueSanity, trendFactorThreshold) {
    metricAggregates.forEach((aggregate) => {
        const { group, instance, metric, type, attributes: { hmi: hmiAttributes = null } = {} } = aggregate;
        if (!hmiAttributes)
            return;
        const hmiName = camel(group, metric, type);
        const getter = () => {
            return instance.get().then((value) => {
                return value === null ? null : sanity(value, valueSanity[metric] || valueSanity.default);
            });
        };
...
export function manage(config, data) {
    ...
    metricAggrgatesHmi(metricAggregates, histories, hmiServer, unitMap, valueSanity, trendFactorThreshold);
}
//# sourceMappingURL=metric-aggregates.js.map