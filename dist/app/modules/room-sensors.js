...
function createSensor(sensor) {
    const { host, port, metrics } = sensor;
    try {
...
function createRoomSensors(roomSensors, metricSchedule, scheduler, security) {
    return roomSensors.map((sensor) => {
        const { disable = false, host, metrics = [], name } = sensor;
        if (disable || !name || !metrics.length)
            return null;
...
export function create(config, data) {
    const { globals: { metricSchedule }, 'room-sensors': roomSensorsConfig } = config;
...
function roomSensorsHmi(roomSensors, histories, hmiServer, unitMap, valueSanity, trendFactorThreshold) {
    roomSensors.forEach((sensor) => {
        const { name, instance, metrics, attributes: { hmi: hmiAttributes = null } = {} } = sensor;
        setUpConnectionHmi(sensor, 'room-sensor', hmiServer);
        if (!hmiAttributes)
            return;
        metrics.forEach((metric) => {
            // don't show pressure for single rooms, except für metrics shown on global
            if (hmiAttributes.section !== 'global'
                && metric === 'pressure')
                return;
            ...
            const handleValue = (value) => {
                ...
                if (typeof value === 'number') {
                    return sanity(value, valueSanity[metric] || valueSanity.default);
                }
...
export function manage(config, data) {
    const { hmi: { trendFactorThreshold, unitMap, valueSanity } } = config;
    const { allMovementGroup, histories, hmiServer, prometheus, roomSensors } = data;
    ...
    roomSensorsHmi(roomSensors, histories, hmiServer, unitMap, valueSanity, trendFactorThreshold);
    allMovementGroupHmi(allMovementGroup, hmiServer);
}
//# sourceMappingURL=room-sensors.js.map