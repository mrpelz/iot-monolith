...
function securityLightKill(security, allLightsGroup) {
    security.on('change', () => {
        if (!security.armed || security.triggered)
            return;
...
export function manage(config, data) {
    const { globals: { entryDoorTimeout, entryDoorMessage } } = config;
    const { allLightsGroup, doorSensors, hmiServer, httpHookServer, prometheus, security, telegram, ufiDisplay } = data;
