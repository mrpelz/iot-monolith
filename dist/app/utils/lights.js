...
export function coupleDoorSensorToLight(lights, doorSensors, lightName, doorSensorName) {
...
export function coupleDoorSensorToLightTimeout(lights, doorSensors, lightName, doorSensorName, timeout = 0) {
    const lightMatch = lights.find((light) => {
        return light.name === lightName;
    });
    const doorSensorMatch = doorSensors.find((sensor) => {
...
export function coupleRfSwitchToLight(lights, rfSwitches, lightName, rfSwitchName, rfSwitchState) {
...
export function coupleRfSwitchToLightIncrease(lights, rfSwitches, lightName, rfSwitchName, rfSwitchState) {
    const lightMatch = lights.find(({ name }) => {
        return name === lightName;
    });
    const rfSwitchMatch = rfSwitches.find(({ name }) => {
    ...
    rfSwitchInstance.on(rfSwitchState, (repeated) => {
        resolveAlways((!lightInstance.power || repeated)
            ? lightInstance.increase(true)
            : lightInstance.setPower(false));
    });
}
export function coupleRfToggleToLight(lights, rfSwitches, lightName, rfSwitchName, rfSwitchState) {
    const lightMatch = lights.find(({ name }) => {
        return name === lightName;
    });
    const rfSwitchMatch = rfSwitches.find(({ name }) => {
    ...
    const { instance: rfSwitchInstance } = rfSwitchMatch;
    rfSwitchInstance.on(rfSwitchState, (repeated) => {
        resolveAlways((repeated && lightInstance.power)
            ? lightInstance.setPower(false)
            : lightInstance.setPower(true));
    });
}
export function coupleRoomSensorToLight(lights, roomSensors, lightName, roomName) {
...
export function coupleRfSwitchesToLightPermutations(lights = [], rfSwitches = [], lightsAll = [], rfSwitchesAll = [], lightsSet = []) {
