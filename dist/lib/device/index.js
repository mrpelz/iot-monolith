...
class Service extends EventEmitter {
    ...
    constructor(identifier, device) {
        const { state: { services } } = device;
        services.forEach((previousDeviceService) => {
            const { state: { identifier: previousIdentifier } } = previousDeviceService;
            if (previousIdentifier.equals(identifier)) {
                throw new Error(`cannot use the same identifier for multiple services (${identifier})`);
            }
        });
        super();
        this.state = {
    ...
    ingestEvent(payload) {
        ...
        if (!serviceIdentifier.equals(this.state.identifier))
            return;
...
export class Device extends EventEmitter {
     ...
     */
    constructor(options) {
        const { transport, identifier, keepAlive = 2000 } = options;
        super();
        /**
         * @type {Logger}
         */
        ...
        rebind(this, 'getService', 'matchDataToRequest', 'onOnlineChange', 'removeService', 'request');
    }
    /**
    ...
    _writeToTransport(requestIdentifier, serviceIdentifier, payload = null) {
        this.transport.writeToTransport(Buffer.concat([
            requestIdentifier,
            serviceIdentifier,
            ...(payload ? [payload] : [])
        ]));
