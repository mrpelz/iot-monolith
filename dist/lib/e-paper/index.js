...
export class EPaper {
    constructor(options = {}) {
        const { hmiServer = null, meta = {}, scheduler = null, update = null, updateOffset = 0, url } = options;
    ...
    _setUpHmiService(hmiServer, scheduler, update, updateOffset) {
        ...
        new RecurringMoment({
            scheduler,
            offset: updateOffset
        }, every.parse(update)).on('hit', this._postValues);
    }
    _postValues() {
        const { hmiService, isActive, log, meta, url } = this._ePaper;
        if (!isActive)
            return;
        hmiService.list(true).then((response = []) => {
            const values = response.reduce((acc, { name, value }) => {
                acc[name] = value;
                return acc;
            }, {});
