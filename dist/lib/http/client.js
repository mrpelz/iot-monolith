...
export function httpClient(method, url = {}, options = {}, data) {
    const { auth, hostname, pathname = '/', port: specifiedPort, protocol, search } = (typeof url === 'string') ? new URL(url) : url;
    if (!hostname) {
        ...
        req.on('response', (res) => {
            ...
            res.on('end', () => {
                ...
                if (res.statusCode < 200 || res.statusCode > 299) {
                    reject(new Error(`failed to load page, status code: ${res.statusCode}, content: ${result.toString()}`));
                }
                else {
                    resolve(result);
                }
