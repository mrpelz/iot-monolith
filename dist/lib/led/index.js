...
function createAnimationPayload(from, to, duration, gamma) {
    ...
    return Buffer.concat(animation.map((frame) => {
...
export class LedDriver extends MessageClient {
    constructor(options = {}) {
        const { host = null, port = null, channels = 0, hasIndicator = true } = options;
    ...
    _set(channel, payload) {
        const { state: { isConnected } } = this._reliableSocket;
...
export class LedLight extends Base {
    constructor(options = {}) {
        super();
        const { driver = null, duration = defaultAnimationDuration, gamma = defaultGamma, useChannel, steps = defaultBrightnessSteps } = options;
        if (!driver
            || duration === undefined
            || gamma === undefined
    ...
    setBrightness(input, duration) {
        ...
        const { duration: defaultDuration, gamma, log, setChannel } = this._ledLight;
        ...
        let payload;
        payload = createAnimationPayload(this.brightness, this.brightnessSetpoint, animationDuration, gamma);
        if (!payload) {
            payload = Buffer.from([
                writeNumber(0, 4),
                writeNumber(cycle, 1)
        ...
        // = 1279 bytes
        return setChannel(Buffer.concat([
            writeNumber(cycle, 1),
            payload
        ])).then((result) => {
            if (result !== cycle) {
...
export class RGBLed {
    constructor(options = {}) {
        const { driver = null, r, g, b, duration = defaultAnimationDuration, gamma = defaultGamma, presets = defaultRGBPresets } = options;
    ...
    setPreset(name, duration) {
        ...
        if (!preset)
            return Promise.reject(new Error('no preset with this name'));
        const { r, g, b } = preset;
