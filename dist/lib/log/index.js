...
const { isProd = false, logLevel = 7, logTelegram = false } = global;
...
export class Logger {
    ...
    _log(opts, usedPrefix) {
        const { _name, _prefixes } = this;
        const { level, head, value = null, attachment = null, telegram = null } = opts;
        ...
        const messageBody = (`${head}${value === null ? '' : ` = ${value}`}`);
        const messageAttachment = (`${attachment === null ? '' : indent(attachment)}`);
