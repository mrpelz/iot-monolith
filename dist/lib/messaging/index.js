...
function prepareMessageTypes(types) {
    return types.map((msg) => {
        const { eventName = null, eventParser = (x) => { return x; }, generator = (x) => { return x; }, head = emptyBuffer, name, parser = (x) => { return x; }, tail = emptyBuffer, timeout = defaultTimeout } = msg;
        return {
            eventName,
            eventParser,
            generator,
...
export class MessageClient extends ReliableSocket {
    constructor(options = {}) {
        const { host = null, port = null, lengthBytes = 1, timeout = 2000, messageTypes = [] } = options;
    ...
    _handleDisconnection() {
        const { log, state: { calls } } = this._messaging;
    ...
    _handleResponse(data) {
        const { state: { calls } } = this._messaging;
    ...
    request(name, data = emptyBuffer, quietParse = false) {
        ...
        const { log, state, state: { calls }, types } = this._messaging;
        if (!isConnected) {
