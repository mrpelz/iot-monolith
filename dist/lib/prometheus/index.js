...
export class Prometheus {
    ...
    metric(name, labels, handler, timeHandler = null) {
        ...
        metrics.push(() => {
            return resolveAlways(handler()).then((value) => {
                ...
                return drawMetric(prefix, name, labelString, drawValue(value), time.getTime());
            });
        });
    }
    pushMetric(name, labels) {
        ...
        metrics.push((test) => {
            ...
            return Promise.resolve(drawMetric(prefix, name, labelString, drawValue(value), (time || now).getTime()));
        });
        return {
            push
        };
    ...
    slowMetric(name, labels, handler) {
        ...
        metrics.push(async () => {
            ...
            const result = await drawMetric(prefix, name, labelString, drawValue(lastValue), Date.now());
            return result;
        });
    }
}
