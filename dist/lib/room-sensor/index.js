...
function prepareMetricHandlers(metrics) {
    ...
    const types = metrics.map((name) => {
        ...
        const { bytes, event, head, sanity: sanityOptions = {}, timeout, tolerateNull } = options;
        const parser = (input) => {
            if (!input.length) {
                if (tolerateNull) {
                    return null;
...
function getCaches(metrics) {
    return arraysToObject(metrics, metrics.map((name) => {
...
export class RoomSensor extends MessageClient {
    ...
    requestMetric(metric) {
        const { state: { isConnected, connectionTime } } = this._reliableSocket;
