import { arrayPadLeft, bufferToBoolean, concatBytes, numberToDigits, swapByte } from '../utils/data.js';
...
function stringToBytemap(input) {
    ...
    return input.padStart(displayLength, ' ').split('').map((char) => {
        ...
        const sign = (lc === ' ') ? empty : (letterMap[char]
            || letterMap[lc]
            || letterMap[uc]
...
export class SevenSegment extends MessageClient {
    ...
    setSlideshow(input, delay = animationDelay) {
        ...
        const slides = [...((typeof input === 'string')
                ...
                : input).map(stringToBytemap), emptyDisplay];
