...
export class PersistentSocket extends Base {
    constructor(options = {}) {
        ...
        const { delimiter = null, host = null, keepAlive = {}, lengthPreamble = 0, port = null } = options;
        if (!host || !port || !(lengthPreamble > 0 || delimiter)) {
            throw new Error('insufficient options provided');
        }
        const { data = null, receive = false, send = false, time = null, useNative = true } = keepAlive;
        if (send && (!time || !data)) {
    ...
    _onConnection() {
        const { log, state, options: { keepAlive }, socket } = this._persistentSocket;
        const now = Date.now();
    ...
    _doConnect() {
        const { log, state, options: { host, port }, socket } = this._persistentSocket;
    ...
    write(input) {
        const { state: { isConnected }, options: { lengthPreamble, delimiter }, log, socket } = this._persistentSocket;
        if (!isConnected) {
...
export class ReliableSocket extends Base {
    constructor(options = {}) {
        super();
        const { host = null, port = null, lengthPreamble = 1, keepAlive = 2000 } = options;
        ...
        rebind(this, '_connect', '_handleReadable', '_notifyConnect', '_notifyDisconnect', '_onConnection', '_onDisconnection', '_sendKeepAlive');
        this._init();
    }
    _init() {
        const { options: { keepAlive }, keepAliveTimer, messageTimer, disconnectTimer, connectDebounceTimer } = this._reliableSocket;
    ...
    _connect() {
        const { options: { host, port }, state: { isConnected, shouldBeConnected }, log } = this._reliableSocket;
    ...
    _onConnection() {
        const { state: { isConnected }, disconnectTimer, connectDebounceTimer, keepAliveTimer, log } = this._reliableSocket;
        if (isConnected)
            return;
    ...
    _onDisconnection() {
        const { state: { isConnected, shouldBeConnected }, messageTimer, disconnectTimer, connectDebounceTimer, keepAliveTimer, log } = this._reliableSocket;
        if (!isConnected)
            return;
    ...
    _read() {
        const { messageTimer, socket, options: { lengthPreamble }, log } = this._reliableSocket;
    ...
    _sendKeepAlive() {
        if (!this._reliableSocket.state.isConnected)
            return;
        this.write(Buffer.from([0xFF]));
    }
    _setUpSocket() {
        const { options: { keepAlive }, socket } = this._reliableSocket;
        socket.setNoDelay(true);
        if (keepAlive) {
    ...
    write(input) {
        const { options: { lengthPreamble }, state: { isConnected }, socket, log } = this._reliableSocket;
        if (!isConnected) {
            throw new Error('socket is not connected!');
        }
