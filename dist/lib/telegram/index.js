...
function telegramRequest(host, token, method, payload) {
    return post(`https://${host}/bot${token}/${method}`, Buffer.from(JSON.stringify(payload), 'utf8'), telegramHttpOptions).then((response) => {
...
class Message {
    static createPayload(chat = {}, options = {}) {
        const { html, inlineKeyboard, markdown, notification = true, pagePreviews = true, responseToId, text } = options;
        if (!text)
    ...
    static createOutgoing(client, chat, options) {
        ...
        return client.request('sendMessage', Message.createPayload(chat, options)).then((payload) => {
            ...
            const { date, from: { id: fromId = null } = {}, message_id: messageId, text } = payload;
            return new Message({
                chat,
                client,
                date,
    ...
    callbackQuery(options) {
        if (this.deleted || !this._inlineKeyboard)
            return;
        const { id } = options;
    ...
    edit(options = {}) {
        ...
        const calls = editKeys.map((key) => {
            ...
            return this._client.request(method, Object.assign({
                message_id: this.id,
                [key]: value
            }, excludeKeys(Message.createPayload(this._chat, options), ...editKeys))).then((payload = {}) => {
...
class Chat {
    ...
    ingestCallbackQuery(payload = {}) {
        const { data, from: { id: fromId = null } = {}, id, message: { message_id: messageId = null } = {} } = payload;
        if (data === undefined)
            return;
        if (this._messages.has(messageId)) {
...
class Client {
    static create(scheduler, token, host, db) {
        ...
        return telegramRequest(host, token, 'getMe', {}).then((payload) => {
            ...
            const { first_name: firstName, id, last_name: lastName, username } = payload;
            return new Client({
                db,
                firstName,
                host,
    ...
    constructor(options) {
        const { db, firstName, host, id, lastName, scheduler, token, username } = options;
    ...
    _onUpdate(result = []) {
        result.forEach((update) => {
            ...
            if (!type)
                return;
            const { [type]: payload, [type]: { chat: { id: chatIdA = null } = {}, message: { chat: { id: chatIdB = null } = {} } = {} } = {} } = update;
