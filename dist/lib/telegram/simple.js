...
export function telegramSend(message) {
    const { telegramToken } = global;
    return post(`https://${host}/bot${telegramToken}/sendMessage`, Buffer.from(JSON.stringify(Object.assign(telegramMessageOptions, {
        chat_id: chatId,
        text: message
    })), 'utf8'), telegramHttpOptions).then((response) => {
