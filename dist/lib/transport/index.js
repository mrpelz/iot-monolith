...
export class TransportDevice {
    ...
    constructor(transport, device) {
        const { state: { devices, identifier: transportIdentifier } } = transport;
        const { state: { identifier: deviceIdentifier } } = device;
        if (transportIdentifier && transportIdentifier.length !== deviceIdentifier.length) {
        ...
        devices.forEach((previousTransportDevice) => {
            const { state: { device: { state: { identifier: previousDeviceIdentifier } } } } = previousTransportDevice;
            if (previousDeviceIdentifier.equals(deviceIdentifier)) {
                throw new Error(`cannot use the same identifier for multiple devices (${deviceIdentifier})`);
            }
        });
        /**
         * @type {{
    ...
    ingestIntoDeviceInstance(identifier, payload) {
        const { transport: { state: { identifier: transportIdentifier } }, device, device: { state: { identifier: deviceIdentifier } } } = this.state;
    ...
    writeToTransport(payload) {
        const { transport, transport: { state: { identifier: transportIdentifier } }, device: { state: { identifier: deviceIdentifier } } } = this.state;
...
export class Transport {
     ...
     */
    constructor(options) {
        const { devices = /** @type {TransportDevices} */ (new Set()), identifier, singleDevice = true } = options;
        ...
        rebind(this, 'addDevice', 'connect', 'disconnect', 'reconnect', 'removeDevice', 'writeToNetwork');
    }
    /**
    ...
    writeToNetwork(deviceIdentifier, payload) {
        throw new Error(`no write method defined in ${this} to process identifier "${deviceIdentifier}" and payload "${payload}"`);
    }
}
/**
...
export class AggregatedTransport {
    ...
    constructor(options, SubTransport, ...transportOptions) {
        const { devices = /** @type {TransportDevices} */ (new Set()), identifier } = options;
        if (!SubTransport
            || !devices
            || !identifier) {
         ...
         */
        this.state = {
            transports: AggregatedTransport.createSubTransports(SubTransport, options, transportOptions),
            devices,
            identifier,
            isConnected: null,
            log: this.log.withPrefix(libName)
        };
        rebind(this, 'addDevice', 'connect', 'disconnect', 'reconnect', 'removeDevice', 'writeToTransport');
    }
    /**
