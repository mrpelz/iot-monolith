...
export class UDPTransport extends Transport {
    ...
    constructor(options) {
        ...
        rebind(this, 'addDevice', 'connect', 'disconnect', 'reconnect', 'removeDevice', 'writeToNetwork', '_connect', '_handleMessage', '_onConnection', '_onDisconnection');
    ...
    _handleMessage(message, remoteInfo) {
        ...
        if (sequenceHandling) {
            ...
            if (!this._checkIncomingSequence(sequence.readUInt8(0)))
                return;
        }
        else {
            payload = message;
