...
export function sanity(input, options) {
    if (input === null)
        return null;
    const { divide = 1, max = Number.POSITIVE_INFINITY, min = Number.NEGATIVE_INFINITY, multiply = 1, offset = 0, round = 0 } = options;
    if (input > max)
        return null;
    if (input < min)
        return null;
    ...
    value *= multiply;
    if (round) {
        value = trimDecimals(value, Number.isInteger(round)
            ? round
            : 0);
    }
    return value;
 ...
 * }[] | null}
 */
export function ledCalc(fromP, toP, duration = 3000, transition = transitions.linear, range = 255, gamma = 2.8, timeStep = 17) {
    /**
     * @param {number} input
     */
    const toPwm = (input) => {
        return Math.round(Math.max(Math.min(gammaCorrect(input, range, gamma), range), 0));
    };
