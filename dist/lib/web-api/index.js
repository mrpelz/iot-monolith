...
function elementNames(elements, key = 'name') {
    ...
    elements.forEach(({ attributes: { [key]: value = null } = /** @type {{ [key: string]: any }} */ ({}) }) => {
        result.add(value);
    });
    return [...result].sort();
}
function elementsInHierarchy(elements, value, key = 'name') {
    return elements.filter(({ attributes: { [key]: is = null } = /** @type {{ [key: string]: any }} */ ({}) }) => {
        return is === value || ((value === undefined || value === null)
            && (is === undefined || is === null));
    });
}
function combineAttributes(elements) {
    return Object.assign({}, ...elements.map((element) => {
...
function findShowSubGroup(elements) {
    ...
    elements.forEach((element) => {
        const { attributes: { group: primary = null, groupLabel: secondary = null } = {} } = element;
    ...
    return elements.map((element) => {
        const { attributes, attributes: { group: primary = null, groupLabel: secondary = null } = {} } = element;
        const group = secondary || primary;
...
function getHierarchy(elements = [], { sections = [], categories = [], groups = [] } = {}) {
    const sectionMap = sections.map((sectionGroup) => {
        return sortElements(sectionGroup.map((sectionName) => {
            ...
            const categoryMap = sortElements(elementNames(sectionElements, 'category').map((categoryName) => {
                const categoryElements = findShowSubGroup(elementsInHierarchy(sectionElements, categoryName, 'category'));
                ...
                const groupMap = sortElements([].concat(...elementNames(categoryElements, 'group').map((groupName) => {
                    ...
                    const { groupLabel, showSubGroup, sortGroup, subGroup, type } = combineAttributes(groupElements);
                    return [{
...
export class WebApi {
    constructor(options = {}) {
        const { host = undefined, port = null, hmiServer = null, scheduler = null, update = null, meta = {} } = options;
        ...
        rebind(this, '_handleIngest', '_handleStream', '_handleList', '_handleValues', '_handleSet');
    ...
    _updateList() {
        const { hmiService, meta: { extensions = {}, sort = {}, strings = {} } } = this._webApi;
        hmiService.list().then((elements) => {
            return JSON.stringify({
                strings,
                hierarchy: getHierarchy([].concat(elements, getExtensions(extensions)), sort)
            }, null, null);
        }).catch(() => { }).then((result) => {
            if (!result)
                return;
    ...
    _handleStream(request, response) {
        ...
        return {
            ...
            openEnd: true,
            handler: Promise.resolve(`: welcome to the event stream\n: client "${name}"\n\n`)
        };
    }
    _handleList(request) {
        const { hmiService, list, meta: { extensions = {}, sort = {}, strings = {} } } = this._webApi;
