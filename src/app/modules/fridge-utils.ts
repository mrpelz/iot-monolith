...
async function fridgeTimer(
  telegram: TelegramState,
  fridge: DoorSensorState,
  fridgeTimeout: number,
  fridgeMessage: string
) {
...
export function manage(
  ...
  const {
    globals: { fridgeTimeout, fridgeMessage },
  } = config;

  const { doorSensors, telegram } = data;
