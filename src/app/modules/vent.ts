...
import { rfSwitchState } from './rf-switches.js';
import { setUpConnectionHmi } from '../utils/hmi.js';

type VentConfig = ApplicationConfig['vent'];
type VentState = VentConfig & { instance: Vent };

export type State = {
  vent: VentState;
};

function createVent(ventConfig: VentConfig) {
...
export function create(
  ...
  const { disable = false, name } = ventConfig;
  ...
  Object.defineProperty(data, 'vent', {
    value: vent,
  });
}

function manageVent(vent: VentState, httpHookServer: HttpServer) {
  if (!vent) return;
  ...
  httpHookServer.route(`/${name}/target`, (request: any) => {
    const {
      urlQuery: { target },
    } = request;
...
async function createHysteresis(
  ...
  fullVentAboveHumidity: number,
  resetVentBelowHumidity: number,
  ventHumidityControlUpdate: string,
  telegram: TelegramState,
  fullVentMessage: string,
  resetVentMessage: string
) {
  if (!vent) return;
  ...
  const ventControlSensor = roomSensors.find((sensor) => {
    ...
    return name === 'ahuOut' && metrics.includes(metric);
  });
  ...
  new RecurringMoment({ scheduler }, every.parse(ventHumidityControlUpdate)).on(
    'hit',
    () => {
...
async function kackButtons(
  ...
  const rfSwitchInstances = rfSwitches
    .filter(({ name }) => {
      return ['duschbadButtonToilet', 'wannenbadButtonToilet'].includes(name);
    })
    .map(({ instance }) => {
      return instance;
    });
...
function ventToPrometheus(vent: VentState, prometheus: Prometheus) {
  if (!vent) return;
...
function ventHmi(vent: VentState, hmiServer: HmiServer) {
  if (!vent) return;
  ...
  hmiTargetUp.on('set', () => {
    const target =
      instance.target === null || instance.target === instance.maxTarget
        ? instance.minTarget
        : instance.target + 1;
  ...
  hmiTargetDown.on('set', () => {
    const target =
      instance.target === null || instance.target === instance.minTarget
        ? instance.maxTarget
        : instance.target - 1;
